...
for kk=1:Sample 
%Sample random POVM (input is deviation from perfect state - should not bee too big otherwise sampling will fail)
POVM=samplepovm(0.1);
%Put POVM elements into optimization vector for fmincon (using cholesky
%decomposition)
M000=ctranspose(chol(nearestSPD(POVM{1,1})));
M001=ctranspose(chol(nearestSPD(POVM{1,2})));
M010=ctranspose(chol(nearestSPD(POVM{1,3})));
M011=ctranspose(chol(nearestSPD(POVM{1,4})));
...
%Sample random initial matrix (input is deviation from perfect starting
%state). 
